...
if selected == 'UNDANGAN':
    st.write('undangan nih buat ayaangkuu❤❤')
    st.write ('') 
    ...
    st.write('Jabatan : suami Anis Muzkiyah') 
    st.write('Masa Jabatan : SELAMANYA') 
    st.write('') 
    st.write('Dengan ini mengucapkan selamat HARI VALENTINE 2024.') 
    st.write('Demikian Kartu ucapan ini mamas buat dengan cinta seluas alam raya terkhusus bidadariku yang cantiiik, yang maniiisss, yang baiikkk, dan tak ada duanya') 
    st.write('') 
    st.write ('') 
    st.write ('Terdanda') 
    st.write('Mamas Ganteng') 
if selected == 'UPLOAD DONG':
    st.write('halaman sebelum terakhir..')
    st.write ('') 
    temp = st.file_uploader("Upload foto cantiknya dooong ayaangkuu 🥰🥰🥰")
    buffer = temp
...
if selected == 'WITH MY LOVE':
    st.write('Haloo ayaangkuu, halaman terakhir niih🥰🥰🥰')
